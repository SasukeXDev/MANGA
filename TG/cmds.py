        ...
        if message.command[1] != "start":
            ...
            if verify_token_memory(user_id, token):
                sts = await message.reply(
                    "Token verified! You can now use the bot."
                )
                ...
                sts = await message.reply(
                    "Invalid or expired token. Requesting a new one..."
                )
            ...
            [
                ...
                [
                    InlineKeyboardButton(
                        "Database", url="https://t.me/undefined"
                    ),
                    InlineKeyboardButton(
                        "Group Chat", url="https://t.me/Crunchyroll_Support"
                    ),
                ],
...
async def on_private_message(client, message):
    ...
    try:
        ...
        await message.reply(
            "Channel does not exist, therefore bot will continue to operate normally"
        )
        return message.continue_propagation()

    except pyrogram.errors.ChatAdminRequired:
        await message.reply(
            "Bot is not admin of the channel, therefore bot will continue to operate normally"
        )
        return message.continue_propagation()

    except pyrogram.errors.UserNotParticipant:
        await message.reply(
            "<b>In order to use the bot you must join it's channel.</b>",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            ' Join Channel ! ', url=f't.me/{channel}'
                        )
                    ]
                ]
            ),
...
@Client.on_message(filters.command(["broadcast", "b"]) & filters.user(Bot.ADMINS))
...
@Client.on_message(filters.command("restart") & filters.user(Bot.ADMINS))
...
@Client.on_message(filters.command("shell") & filters.user(Bot.ADMINS))
async def shell(_, message):
    cmd = message.text.split(maxsplit=1)
    if len(cmd) == 1:
        return await message.reply(
            "<code>No command to execute was given.</code>"
        )
...
@Client.on_message(filters.command("export") & filters.user(Bot.ADMINS))
...
@Client.on_message(filters.command("import") & filters.user(Bot.ADMINS))
...
@Client.on_message(filters.command(["clean", "c"]) & filters.user(Bot.ADMINS))
...
async def queue_msg_handler(client, message):
    ...
    await message.reply(
        f"<blockquote><b><i>Your Queue: {queue.get_count_(message.from_user.id)}\nTotal Queue Size: {int(queue.qsize())+1}</i></b></blockquote>"
    )


@Client.on_message(filters.command(["us", "user_setting", "user_panel"]))
async def userxsettings(client, message):
    ...
    try:
        ...
        button = [
            ...
            [
                InlineKeyboardButton(
                    "⚒ File Type ⚒", callback_data="u_file_type"
                )
            ],
        ]
...
@Client.on_message(
    filters.command(["deltask", "cleantasks", "del_tasks", "clean_tasks"])
)
...
@Client.on_message(filters.command("unsubs"))
